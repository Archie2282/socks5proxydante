...
# This realisation should work on Debian, Ubuntu and CentOS


# Detect Deb users running the script with "sh" instead of bash
...
if [[ -e /etc/sockd.conf ]]; then
	...
	if [[ "$OStype" = 'deb' ]]; then
		# If deb based distro
		apt-get update
		apt-get -y install openssl make gcc wget
	else
		# Else, the distro is CentOS
		yum -y install epel-release
		yum -y install openssl make gcc wget
	fi
fi
# Getting dante 1.4.2
...
internal: 0.0.0.0 port = {port_number}
...
sed -i "s/internal: 0.0.0.0 port = {port_number}/internal: 0.0.0.0 port = $port/g" /etc/sockd.conf
sed -i "s/internal: :: port = {port_number}/internal: :: port = $port/g" /etc/sockd.conf
